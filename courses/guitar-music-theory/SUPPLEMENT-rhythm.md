...
Durations halve as they divide:

| Value | Length in 4/4 | Count it |
|---|---|---|
| Whole note | 4 beats | "1 (2 3 4)" |
| Half note | 2 beats | "1 (2)" |
| Quarter note | 1 beat | "1" |
| Eighth note | ½ beat | "1 &" ("one-and") |
| Sixteenth note | ¼ beat | "1 e & a" ("one-ee-and-uh") |

Two modifiers:
- **Dot** = add half the value again (dotted half = 3 beats; dotted quarter = 1½).
- **Triplet** = squeeze 3 evenly into the space of 2 (count "1-trip-let").

### Time signatures

The top number = beats per measure; the bottom = which value gets the beat.
...
Count eighth notes "1 & 2 & 3 & 4 &": numbers are **downbeats** (hand moving down), "&"s are **upbeats** (hand moving up). **The golden rule of strumming: the hand never stops moving.** Patterns are made by *missing* the strings on purpose, not by stopping. That's why the universal pattern works:
...
2. What does a dot do to a note value?
3. Define syncopation in one sentence.
4. What's the difference between straight and swung eighths, and which does blues use?
5. What is harmonic rhythm?
