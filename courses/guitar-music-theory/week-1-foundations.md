...
- Memorize the natural notes on strings 6 (low E) and 5 (A).
- Understand why the B string breaks the pattern.

### Lecture

**The 12 notes.** Western music uses 12 pitches that repeat in a loop:
...
E str:  E    F    F#   G    G#   A    A#   B    C    C#   D    D#   E
A str:  A    A#   B    C    C#   D    D#   E    F    F#   G    G#   A
```
...
A|--0--2--3--5--7--8--10--12--   (A B C D E F G A)
```

### Exercises (Lab)

1. **(15 min)** Say-and-play strings 6 and 5, naturals only, up and down, until you can do each in under 20 seconds without hesitating.
2. **(10 min)** Random-note drill: write the 7 natural letters on scraps of paper, shuffle, draw one, find it on string 6 within 3 seconds. Repeat for string 5.
3. **(10 min)** Reverse drill: pick a random fret (1–12) on string 6 or 5, name the note.
4. **(10 min)** Find every **G** on strings 6 and 5 (fret 3 and fret 10). Then every **C** (8 and 3). Then every **E** (0/12 and 7).
5. ★ **(10 min)** Start learning string 4 (D): F=3, G=5, A=7, B=9, C=10, D=12.

### Daily drill (do this every day for the rest of the course)
5 minutes: random note name → find it on a random string as fast as possible. Add one new string every 2–3 days: order 6, 5, 4, 3, 2 (string 1 = string 6).

### Self-check
1. Which two letter pairs have no sharp between them?
2. What note is fret 8, string 6? Fret 5, string 5?
3. How is the G→B string pair tuned differently from all the others?
4. What's another name for Db?
...
- Compute any interval above a given note.
- Begin hearing intervals via song associations.

### Lecture

**An interval is the distance between two notes**, measured two ways at once:
...
| 4 | Major 3rd | M3 | "Oh, when the Saints" |
| 5 | Perfect 4th | P4 | "Here Comes the Bride" |
| 6 | Tritone | TT (A4/d5) | *The Simpsons* ("The Simp-") |
| 7 | Perfect 5th | P5 | *Star Wars* main theme |
| 8 | Minor 6th | m6 | *The Entertainer* (pickup) |
| 9 | Major 6th | M6 | "My Bonnie Lies Over the Ocean" |
| 10 | Minor 7th | m7 | *Star Trek* (original theme) |
| 11 | Major 7th | M7 | "Take On Me" (chorus leap) |
| 12 | Octave | P8 | "Somewhere Over the Rainbow" |

**Quality logic:** 2nds, 3rds, 6ths, 7ths come in **major/minor** (minor = one semitone smaller). 4ths, 5ths, octaves are **perfect**. A perfect or minor interval shrunk by one more semitone is **diminished**; a perfect or major interval stretched by one is **augmented**. (That's why 6 semitones has two names: augmented 4th = diminished 5th = tritone.)
...
Root G (string 6 fret 3): the P5 (D) is one string up, two frets up. The M3 (B) is one string up, **one fret back**. These two shapes are the skeleton of every chord you know.
...
### Daily drill
Add 3 minutes of interval singing to your fretboard drill: play a note, name an interval, sing the target, check on the guitar.

### Self-check
1. How many semitones in a major 6th? A tritone?
2. What note is a m3 above E? A P4 above F#?
3. Which interval families are "perfect"?
4. Name the interval: G up to E.
...
- Play the major scale in two movable positions.
- Understand scale degrees (1–7) and their names.
...
**Spelling rule:** a scale uses **each letter exactly once**. That's why G major has F#, not Gb (Gb would give you two G's and no F).

**Scale degrees.** Number the notes 1–7. Names worth knowing: **1 = tonic** (home), **5 = dominant** (tension that points home), **4 = subdominant**, **7 = leading tone** (the half step below the tonic that "leads" to it). When you hear that a melody "wants to resolve," it's usually degree 7 pulling to 1, or 4 pulling to 3.
...
### Self-check
1. Recite the major scale formula.
2. Spell E major. Spell F major.
3. Which degree is the leading tone, and what does it want to do?
4. Why does a scale never repeat a letter name?
...
- Know the sharp/flat order and the "last sharp / second-to-last flat" shortcuts.
- Understand what "being in a key" means.
...
- **Sharps are added in this order:** F C G D A E B ("Father Charles Goes Down And Ends Battle").
- **Flats are added in the reverse order:** B E A D G C F (it spells BEAD-GCF).

**Shortcuts that make this instant:**
- *Sharp keys:* the key is **one half step above the last sharp**. Three sharps = F# C# G# → half step above G# = **A major**.
...
**Enharmonic spelling rule (from Day 1, now resolved):** use the name that fits the key. In G major the seventh note is F# (never Gb); in F major the fourth is Bb (never A#). Sharps for sharp keys, flats for flat keys.
...
1. **(15 min)** Without references, write out the full circle from memory: key → number of sharps/flats → the actual accidentals. Check, then do it again until perfect.
2. **(10 min)** Speed round (write answers, check after): key signature of E? Bb? B? Ab? Which key has 4 sharps? 2 flats?
3. **(15 min)** Transposition lab: take the progression G–C–D–Em. Transpose it to A major and to C major on paper (count the scale degrees: G=1 C=4 D=5 Em=6 → in A: A–D–E–F#m). Play all three versions with any chord shapes you know.
4. **(10 min)** One-string proof: play F major and C major on the E string; identify the single changed note.
5. ★ Draw the circle of fifths from memory on paper. Add the relative minors if you're curious (we formalize them Day 15: each major key's 6th degree).
...
- Know the cross-string shapes for the most important intervals (m3, M3, P4, P5, m7, M7, octave).
- Internalize the B-string shift in practice.
...
B|----------------13-------   (from D-string C: +2 strings +3 frets; also fret 1)
G|-----------5-------------   (from A-string C: +2 strings +2 frets)
D|------10-----------------   (from E-string C: +2 strings +2 frets)
A|--3----------------------
E|--8----------------------
```
...
1. **(15 min)** Octave-chain drill: random note → find it on all 6 strings via octave shapes. 10 notes.
2. **(15 min)** Interval-shape drill: random root on string 6 or 5, random interval → play it. 20 reps.
3. **(10 min)** B-string gauntlet: random root on string 3, random interval → play it on string 2. 10 reps. Every answer must account for the shift.
4. **(10 min)** Ear: have the interval shapes under your fingers, then play random ones eyes-closed and identify by ear before looking.
5. ★ Map the M6 and m6 shapes yourself (M6 = 2 strings up, −1 fret from strings 6/5; derive m6).

### Daily drill (updated)
Your 10-minute drill is now: 4 min note-finding (all strings via octaves) + 3 min interval shapes + 3 min interval ear training.
