...
2. **The octave web:** on a second copy, pick the note G and mark *all* its locations, connecting them with the octave shapes from Day 5. Repeat for C# (forcing you off the easy landmarks).
3. **The interval legend:** draw the fretboard shapes (root + target) for m3, M3, P4, P5, m7, and octave from a string-6 root *and* a string-5 root, plus one example each where the target lands on the B string (showing the +1 shift).
4. **Performance check (record audio or video, 2 min):** someone (or a shuffled list you prepared) calls 12 random note+string pairs; you find each in ≤3 seconds, speaking the answer.

### Rubric (10 pts)
| Pts | Criterion |
|---|---|
| 3 | Map complete and correct from memory (−1 per 2 errors) |
| 2 | Octave web correct, all locations found |
| 2 | Interval legend correct incl. B-string shift examples |
| 3 | Performance check: 10+/12 within time |
...
2. **Three original progressions**, each 8 bars, each in a different key from your grid:
   ...
   - (c) one built on a **falling-5ths chain** of at least four chords.
...
| Pts | Criterion |
|---|---|
| 4 | Grid complete and correct from memory |
| 4 | Progressions are correct as labeled; cadences/devices genuinely function as named |
| 3 | Transpositions correct and played cleanly |
| 2 | Voice-led arrangement stays in window; movement between chords is minimal |
| 2 | Recording complete with spoken analysis |
...
For each song, the write-up is one page max:
- Key (and how you determined it)
- Section-by-section Roman numerals
- Cadences at phrase ends
- **Findings:** anything non-diatonic, named and explained in one sentence each
- One "so what": a sentence on how the harmony serves the song's feeling

### Rubric (15 pts)
| Pts | Criterion |
|---|---|
| 5 | Ear transcription substantially correct; discrepancies honestly reconciled |
| 4 | Numerals/keys correct across all three songs |
| 3 | Non-diatonic events correctly identified and explained |
| 3 | "So what" sentences show musical judgment, not just labeling |
...
### Specification
Compose an original piece with:
1. **Form:** at least two distinct 8-bar sections (e.g., verse + chorus) in a key of your choice (major or minor).
2. **Harmonic requirements** (all must appear, all must be labeled):
...
**Master's bar:** 23+/25, *and* you could re-do the whole project in a different key tomorrow without breaking stride.
