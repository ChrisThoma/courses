...
2. **Four tools, well-shaped** (apply Lecture 2 — descriptions are graded):
   ...
   - `edit_file` — **search/replace semantics from Lab 4**: exact-match old string; error loudly on zero matches ("not found — re-read the file; whitespace must match exactly") and on multiple matches ("matched 3 locations — provide more surrounding context").
