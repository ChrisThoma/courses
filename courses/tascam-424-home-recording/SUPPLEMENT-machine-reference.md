...
2. **Set the MONITOR SELECT (LINE OUT) switch to L-R.** Your headphones now carry the main mix, where your **live input** sits (the channel you are recording, INPUT select = MIC/LINE, fader up).
3. **Raise the TAPE CUE knob for each track you want to hear:** the click on track 4, the foundation on track 1, and so on. These blend the recorded tracks in over the top of your live part.
4. **Set MONITOR LEVEL for a comfortable headphone volume,** then balance by ear: the click clearly audible, the foundation present, your live part loud enough to play in tune. If you hear no tape, check that the EFFECT 2 / TAPE CUE switch is on TAPE CUE (the usual culprit).
...
**Punch-in (fixing a mistake in place), covered Day 11:**
- The 424 MKIII does **auto punch-in/out** with a **rehearsal** mode: you set an in-point and out-point, rehearse the punch without recording, then commit.
- An optional **RC-30P momentary footswitch** plugs into the punch jack so you can punch hands-free while you play.
...
**The fix is a buffer between the guitar and the machine, and a pedal is a buffer.** Most pedals (anything powered, and especially buffered-bypass pedals like classic Boss units) present a high input impedance to your guitar and a **low-impedance, line-friendly output** to whatever comes next. So:

```
  passive guitar ─► (any powered/buffered pedal) ─► 424 LINE input  =  full, strong tone
  passive guitar ─────────────────────────────────► 424 LINE input  =  dull, thin, quiet
```

Practical consequences:

- **Always have at least one pedal (or a DI box) between a passive instrument and the 424.** Even a single always-on buffer, compressor, or overdrive fixes the impedance problem and improves everything downstream.
